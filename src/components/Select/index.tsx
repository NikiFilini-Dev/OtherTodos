...
const Select = observer(({variants, selected, select, stretch=false}: {variants: Variant[], selected?: Variant | string, select: (code: string) => void, stretch?: boolean}) => {
  const list = [...variants]
  ...
  return <div className={classNames({
    ...
    >
      {selected && (
        <React.Fragment>
          {selected.icon && <Icon name={selected.icon} className={styles.icon} />}
    ...
    <div className={styles.list} ref={listRef}>
      {isActive && variants.map(variant => <div className={classNames({
        [styles.variant]: true,
        [styles.active]: selected && typeof(selected) === "object" && selected?.code === variant.code
      })} key={variant.code} onClick={() => select(variant.code)}>
        {variant.icon && <Icon name={variant.icon} className={styles.icon} />}
