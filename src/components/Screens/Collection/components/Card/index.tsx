...
const Card = observer(
  ({
    ...
    const bottomVisible =
      card.date !== null || card.comments.length > 0 || card.files.length > 0
        ...
        {card.subtasks.length > 0 && (
          <div className={styles.progressWrapper}>
            ...
            <div
              ...
              style={
                { "--donePercent": `${card.donePercent}%` } as CSSProperties
              }
            />
          </div>
        )}
        {!!(card.date !== null || tags.length) && (
          ...
          <div className={styles.tags}>
            {tags.map(tag => (
              <div
                ...
                style={{ "--color": ColorsMap[tag.color] } as CSSProperties}
                key={"card_tag_" + tag.id}
              >
                {tag.name}
              </div>
            ))}
          ...
          <div className={styles.bottom}>
            {card.files.length > 0 && (
              <div className={styles.count}>
                <AttachmentIcon /> {card.files.length}
              </div>
            )}
            {card.comments.length > 0 && (
              <div className={styles.count}>
                <ChatIcon /> {card.comments.length}
              </div>
            )}
            ...
            {card.date && card.date !== DateTime.now().toFormat("M/d/yyyy") && (
              <div
                className={classNames({
                  [styles.date]: true,
                  [styles.expired]:
                    DateTime.fromFormat(card.date, "M/d/yyyy") < DateTime.now(),
                })}
              >
                <Icon name={"time"} />
