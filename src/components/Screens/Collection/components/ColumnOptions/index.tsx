...
type Props = {
  ...
  triggerRef: any
  menuRef: any
  assignedColumn?: boolean
}

const ColumnOptions = observer(({ column, triggerRef, menuRef, assignedColumn }: Props) => {
  const {
    collectionsStore: { deleteColumn, deleteAssignedColumn },
  }: IRootStore = useMst()

  const onDelete = () => {
    if (assignedColumn) deleteAssignedColumn(column.id)
    else deleteColumn(column.id)
  }
            ...
            {ColorNames.map(name => {
              return (
                <div
                  ...
                  onClick={() => column.setColor(name)}
                  style={{ "--color": ColorsMap[name] } as CSSProperties}
                />
              )
            })}
          </div>
