...
const Subtask = observer(({ subtask }: { subtask: ISubtask }) => {
  ...
  const onKeyUp = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Backspace" && lastText.length === 0) {
      e.preventDefault()
      const prev = subtask.task.subtasks.find(st => st.index === subtask.index-1)
  ...
  return <div key={subtask.id} className={styles.subtask}>
    <Checkbox
...
const Task = observer(({ task, active = false, onConfirm, onReject, expired, newPrompt = false }) => {
  ...
  React.useEffect(() => {
    ...
    // @ts-ignore
    editorRef.current.addEventListener("change", (e: Event & { detail: { original: string } }) => {
    ...
    <TaskContext.Provider value={taskEmitter}>
      ...
      >
        <div className={styles.line}>
          ...
          {task.event && (
            <span
              ...
              style={{ "--tag-color": task.event.tag?.color } as CSSProperties}
            >
            <CalendarWeekIcon />
          </span>
          )}
        ...
        {task.subtasks.length > 0 && (
          <div className={styles.line}>
            <span className={styles.subtasksProgressInfo}>{task.doneSubtasks.length}/{task.subtasks.length}</span>
            <div className={styles.subtasksProgress}
                 style={{ "--donePercent": `${task.progress}%` } as CSSProperties} />
          </div>
        )}
        <div
          className={classNames({
        ...
        >
          ...
          <div className={styles.noteWrapper}>
            ...
            <div className={styles.subtasks}>
              {task.subtasks.map(st => <Subtask subtask={st} key={st.id} />)}
              <div className={styles.addNew} onClick={onAddSubtaskClick}>+ Добавить подзадачу</div>
            </div>
          </div>
        </div>
        <Tags task={task} />
