...
const DaySelector = observer(() => {
  ...
  return <div className={styles.calendar}>
    <div className={styles.monthsSelector}>
      <span className={styles.changeMonth} onClick={() => setMonthOffset(monthOffset-1)}>
        {capitalizeFirstLetter(getMonth(calendarReferenceDate.getMonth()-1))}
      </span>
      <span className={styles.currentMonth}>
        ...
        {calendarReferenceDate.getFullYear() !== new Date().getFullYear() && (
          <span className={styles.yearHint}> ({calendarReferenceDate.getFullYear()})</span>
        )}
      </span>
      <span className={styles.changeMonth} onClick={() => setMonthOffset(monthOffset+1)}>
        {capitalizeFirstLetter(getMonth(calendarReferenceDate.getMonth()+1))}
