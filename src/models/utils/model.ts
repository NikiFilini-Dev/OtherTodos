...
const getDefaultActions = (self: IStateTreeNode) => {
  ...
  const actions: {
    [Property in keyof typeof info.properties]: (
      val: typeof info.properties[Property],
    ) => void
  } = {}
  ...
  Object.keys(info.properties).forEach(name => {
    ...
    actions[actionName] = (val: typeof type) =>
      (((self as unknown) as typeof info.properties)[name] = val)
