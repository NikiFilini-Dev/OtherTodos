import { getParent, IMSTArray, Instance, ISimpleType, types } from "mobx-state-tree"
...
export const HabitColors = ["blue", "yellow", "green", "red", "lightblue", "purple"]
export type HabitColor = "blue" | "yellow" | "green" | "red" | "lightblue" | "purple"
...
export const HabitIcons: HabitIcon[] = ["bookmark", "ship", "ball", "fire", "camera",
  "win", "present", "pool", "book", "wallet"]
export type HabitIcon = "bookmark" | "ship" | "ball" | "fire" | "camera" | "win" |
  "present" | "pool" | "book" | "wallet"
...
const Habit = types
  ...
  .actions(self => {
    ...
    actions.toggleWeeklyDay = (val: CustomRange<1, 8>) => {
      if (self.weeklyDays.includes(val)) self.weeklyDays.splice(self.weeklyDays.indexOf(val), 1)
    ...
    actions.toggleMonthlyDay = (val: CustomRange<1, 32>) => {
      if (self.monthlyDays.includes(val))  self.monthlyDays.splice(self.monthlyDays.indexOf(val), 1)
